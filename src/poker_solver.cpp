...
std::optional<TwoPair> getTwoPair(const Cards &cards)
{
    ...
    auto kicker = highCard(static_cast<uint16_t>(~(static_cast<uint16_t>(firstPair) | static_cast<uint16_t>(secondPair)) & sameCards));
...
std::optional<Tripps> getTripps(const Cards &cards)
{
    ...
    return { Tripps {highCard(potentialTripps), CardRanks(potentialTripps ^ cards.internalRepresentation() )}};
}
...
std::optional<FullHouse> getFullHouse(const Cards &cards)
{
    ...
    if(numTripps == 1) {
        ...
        FullHouse f;
        f.mTripps = static_cast<CardRank>(potentialTripps);
        f.mPair = static_cast<CardRank>(highestPair);
        return { f };
    }

    if(numTripps == 2) {
        FullHouse f;
        f.mTripps = highCard(potentialTripps);
        f.mPair = static_cast<CardRank>(potentialTripps ^ static_cast<uint16_t>(f.mTripps));
        return { f };
    }
...
std::optional<Quads> getQuads(const Cards &cards)
{
    ...
    if(cardRankSet.find(potentialQuad) != cardRankSet.end()) {
        Quads q;
        q.mQuads = potentialQuad;
        auto kickers = xorSuits(cards);
        q.mKicker = highCard(kickers);
        return { q };
    }
