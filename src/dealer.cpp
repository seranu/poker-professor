...
void Dealer::dealFlop()
{
    ...
    PP_LOG_INFO("Flop: %s", mBoard.mFlop->toString().c_str());
}

void Dealer::dealTurn()
{
    mBoard.drawTurn(mDeck);
    PP_LOG_INFO("Turn: %s", mBoard.mTurn->toString().c_str());
}

void Dealer::dealRiver()
{
    mBoard.drawRiver(mDeck);
    PP_LOG_INFO("River: %s", mBoard.mRiver->toString().c_str());
}
...
std::vector<double> Dealer::evaluate()
{
    ...
    for(decltype(results.size()) i = 0; i < results.size(); i++) {
        PP_LOG_INFO("Player %s: equity: %.3f", mPlayers[i].toString().c_str(), results[i]);
    }

    return results;
}

void Dealer::doOneRound()
{
    mDeck.reset();
    dealPlayerCards();
    dealFlop();
